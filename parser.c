...
/*   Updated: 2025/03/11 01:34:03 by malde-ch         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	check_arguments(int argc, char **argv)
{
	...
	while (i < argc)
	{
		...
		{
			ft_putstr_fd
("Error: All arguments must be positive numbers smaller than MAX_INT\n", 2);
			return (1);
		}
		if (i == 1 && ft_atoi(argv[i]) >= PHILO_MAX + 1)
		{
			ft_putstr_fd
("Error: Number of philosophers must be smaller than PHILO_MAX (200)\n", 2);
			return (1);
		}
		i++;
	}
...
int	parser(int argc, char **argv, t_config *config)
{
	...
	if (check_arguments(argc, argv))
		return (1);
	config->nb_philosophers = ft_atoi(argv[1]);
